...
export async function registerRoutes(
  ...
  app.delete("/api/day-slots/:daySlotId/items/:itemId", async (req, res) => {
    try {
      ...
      // Check if this is an auto-scheduled item — if so, create an exception
      const daySlot = await storage.getDaySlotById(daySlotId);
      const menuItem = await storage.getMenuItemById(itemId);
      if (daySlot && menuItem && menuItem.scheduleType !== "manual") {
        await storage.addScheduleException(itemId, daySlot.date);
      }
  ...
  app.get("/api/menu-items/:itemId/exceptions", async (req, res) => {
    try {
      const itemId = parseInt(req.params.itemId);
      if (isNaN(itemId)) {
        return res.status(400).json({ error: "Invalid item ID" });
      }
      const exceptions = await storage.getScheduleExceptions(itemId);
      res.json(exceptions);
    } catch (error) {
  ...
  app.post("/api/menu-items/:itemId/exceptions", async (req, res) => {
    try {
      const itemId = parseInt(req.params.itemId);
      if (isNaN(itemId)) {
        return res.status(400).json({ error: "Invalid item ID" });
      ...
      const exception = await storage.addScheduleException(itemId, new Date(date));
      ...
      res.status(500).json({ error: "Failed to add schedule exception" });
    }
  });

  app.delete("/api/menu-items/:itemId/exceptions/:date", async (req, res) => {
    try {
      const itemId = parseInt(req.params.itemId);
      if (isNaN(itemId)) {
        return res.status(400).json({ error: "Invalid item ID" });
      ...
      await storage.removeScheduleException(itemId, new Date(dateStr));
      res.status(204).send();
    } catch (error) {
      console.error("Error removing schedule exception:", error);
      res.status(500).json({ error: "Failed to remove schedule exception" });
    }
  });
  ...
  const createMenuItemSchema = z.object({
    ...
    // Schedule fields
    scheduleType: z.enum(["manual", "one_off", "daily", "weekdays", "weekends", "custom_days"]).default("manual"),
  ...
  app.post("/api/menu-items", async (req, res) => {
    try {
      ...
      const item = await storage.createMenuItem({
        ...
        cutoffLeadHours: data.cutoffLeadHours,
        ...
        oneOffDate: data.oneOffDate ? new Date(data.oneOffDate) : null,
        isScheduleActive: data.isScheduleActive,
      });
  ...
  const updateMenuItemSchema = z.object({
    ...
    servingOptions: z.array(servingOptionInputSchema).optional(),
    // Schedule fields
    scheduleType: z.enum(["manual", "one_off", "daily", "weekdays", "weekends", "custom_days"]).optional(),
    cutoffLeadHours: z.number().int().min(1).optional(),
    scheduleDays: z.array(z.number().int().min(0).max(6)).nullable().optional(),
    scheduleStartDate: z.string().nullable().optional(),
    scheduleEndDate: z.string().nullable().optional(),
    oneOffDate: z.string().nullable().optional(),
    isScheduleActive: z.number().int().min(0).max(1).optional(),
  });
  ...
  app.patch("/api/menu-items/:itemId", async (req, res) => {
    try {
      ...
      const { allergenIds, ingredientIds, servingOptions, imageUrl, scheduleStartDate, scheduleEndDate, oneOffDate, ...itemData } = validated;

      // Convert date strings to Date objects for schedule fields
      const updatePayload: any = { ...itemData };
      if (scheduleStartDate !== undefined) {
        updatePayload.scheduleStartDate = scheduleStartDate ? new Date(scheduleStartDate) : null;
      }
      if (scheduleEndDate !== undefined) {
        updatePayload.scheduleEndDate = scheduleEndDate ? new Date(scheduleEndDate) : null;
      }
      if (oneOffDate !== undefined) {
        updatePayload.oneOffDate = oneOffDate ? new Date(oneOffDate) : null;
      }

      const item = await storage.updateMenuItem(itemId, updatePayload);
      if (!item) {
  ...
  app.post("/api/orders", async (req, res) => {
    try {
      ...
      const orderItemsData = data.items.map((item) => ({
        ...
        fulfillmentDate: item.fulfillmentDate ? new Date(item.fulfillmentDate) : null,
